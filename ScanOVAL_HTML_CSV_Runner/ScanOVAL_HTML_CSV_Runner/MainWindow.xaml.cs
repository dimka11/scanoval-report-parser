...
#pragma warning restore CA1712 // Do not prefix enum values with type name
    ...
    public partial class MainWindow : Window
    {
        ...
        private void TextBox_Drop(object sender, DragEventArgs e)
        {
            if (e.Data.GetDataPresent(DataFormats.FileDrop))
            {
                ...
                if (files != null && files.Length > 0)
                {
                    ...
                    var dir_path = files[0].Replace(@"\", "/");
                    textBoxFolder.Text = dir_path.Remove(dir_path.Length - 1);
                }
            }
        }
        ...
        private void SelectDirBtn_Click(object sender, RoutedEventArgs e)
        {
            ...
            dlg.InputPath = userDocumentsPath;
            if (dlg.ShowDialog() == true)
            {
                textBoxFolder.Text = userDocumentsPath.Replace(@"\", "/");
            }

        }

        private void SelectScriptPathBtn_Click(object sender, RoutedEventArgs e)
        {
            var userDocumentsPath = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);

            OpenFileDialog choofdlog = new OpenFileDialog();
            choofdlog.InitialDirectory = userDocumentsPath;
            choofdlog.Filter = "Python (*.py)|*.py";
            ...
            if (choofdlog.ShowDialog() == true)
            {
                ...
                textBoxScriptPath.Text = sFileName.Replace(@"\", "/");
            }
        }
    }
}
